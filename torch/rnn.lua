...
cmd:option('-networkType', 'seqlstm', 'Network type')
...
elseif networkType == 'fastlstm' then -- ( no peephole connections)
   rnn = nn.FastLSTM(inputSize, hiddenSize)
   rnn = nn.Sequential()
      :add(nn.Sequencer(rnn))
      :add(nn.SelectTable(-1))
elseif networkType == 'seqlstm' then -- like fastlstm but faster ( no peephole connections)
   rnn = nn.SeqLSTM(inputSize, hiddenSize)
   rnn = nn.Sequential()
      :add(nn.Sequencer(nn.Copy(nil,nil,true)))
      :add(nn.Sequencer(nn.View(1,batchSize,inputSize)))
